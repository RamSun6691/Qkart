...
const Checkout = () => {
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      setAddresses({
        selected: addresses.selected === addressId ? '' : addAddress.selected,
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      if (e.response) {
        ...
        enqueueSnackbar(
          "Could not place order. Check that the backend is running, reacheable and return valid JSON.",
          {
            variant: "error",
          }
        );
            ...
            <Box>
              ...
              {!addresses.all.length ? <Typography my="1rem">
                  No addresses found for this account. Please add one to proceed
                    ...
                    className={addresses.selected===_id?'address-item selected':"address-item not-selected"}
                   
                      >
                    <p onClick={() => {
                      // console.log('selected address',_id);
                        setAddresses({
                          ...addresses,
                          selected: _id,
                        });
                    ...
                    <Button startIcon={<Delete />} onClick={() => deleteAddress(token, _id)}>Delete</Button>
                  </div>
                )
              }
            </Box>
